...
トップページのカード説明、カードラベル、各タイトルページのサマリー、ページラベル、概要、今後の予定、活動記録、メディア紹介は `content/games/<TitleId>.md` で編集します。

Markdown内の改行は概要本文に反映され、`今後の予定`、`活動記録`、`メディア紹介` の箇条書きは日付付きのリストとして表示されます。
...
- NyctoType紹介ページには、メディア向けプレスキットとピッチデッキへの資料リンクを並べて掲載します。
- リンク文言は各言語のNyctoType Markdownにある `presskit` と `pitchdeck` 相当の節で管理します。`kicker` を指定すると、資料リンクの小ラベルも変更できます。
