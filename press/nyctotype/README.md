...
| NyctoType-07-break_JP.png | 3840 × 2160 | ユーザー追加 / 相手のコマを破壊するアビリティ | 1200 × 675 WebP |
| NyctoType-07-break_ENG.png | 3840 × 2160 | ユーザー追加 / 相手のコマを破壊するアビリティ | 1200 × 675 WebP |
| NyctoType-08-damage_JP.png | 3840 × 2160 | ユーザー追加 / ダメージ発生時の画面演出 | 1200 × 675 WebP |
| NyctoType-08-damage_ENG.png | 3840 × 2160 | ユーザー追加 / ダメージ発生時の画面演出 | 1200 × 675 WebP |
...
- スクリーンショットは日本語UI・英語UIの8場面、計16枚。ページでは選択中の言語に対応する8枚だけを表示します。Unityのデバッグ表示は含みません。
...
- 最新情報: 東京ゲームショウ2026（2026年9月17日～21日）は幕張メッセ ホール10／ブース10-E17に出展。一般向けSteam体験版は2026年9月下旬に公開予定。
- 2026-09-15付の日本語・英語プレスリリースを、素材ZIPとは別の平文TXTとしてトップから直接ダウンロードできるようにします。
...
- 9月15日の国内メディア送付用メール案は `press-email-2026-09-15-ja.txt` で管理します。
- Steamの表示（確認時点）は発売日未発表、3言語、開発元Bogosor Studioです。プレスキットはユーザー承認の新しい情報とBogosorGames表記を優先しました。Steamの更新は別作業です。

## 配布と公開

ZIPは日本語版と英語版を分け、各ZIPに対応言語のスクリーンショット8枚、共通のロゴ・キーアート5枚、対応言語のTXT 1点（計14ファイル）のみを明示的に収録します。ビルド、デモの個別リンク、編集元、内部資料、プレビュー、トレーラーは入りません。
