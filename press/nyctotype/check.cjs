...
async function check() {
  ...
  for (const value of ['2026年9月15日','2026年9月17日（木）～21日（月・祝）','東京ゲームショウ2026','ホール10／10-E17','一般向け体験版を2026年9月下旬にSteamで公開予定','コマごとに異なる生成・移動コスト','アビリティをいつ使うか','TGCA（Top Game Creators Academy）第1期助成プロジェクト','BogosorGames',data.steam,data.website,data.email]) assert(pressRelease.includes(value));
  const normalizedPressRelease = pressRelease.replace(/\r\n/g, '\n');
  assert(normalizedPressRelease.includes('2026年9月15日\n\n報道関係者各位\n\nBogosorGames'));
  assert(normalizedPressRelease.includes(['タイトル：NyctoType','開発元：BogosorGames','ジャンル：戦略性タイピングゲーム','プラットフォーム：PC（Windows）／Steam','プレイ人数：1〜2人（オンライン対戦）','発売予定：2027年にアーリーアクセス配信予定'].join('\n\n')));
  ...
  for (const value of ['September 15, 2026','September 17-21, 2026','Tokyo Game Show 2026','Hall 10 / 10-E17','Public Steam Demo Planned for Late September 2026',"each piece's different creation and movement costs",'when to use its abilities','TGCA (Top Game Creators Academy)','BogosorGames',data.steam,data.website,data.email]) assert(pressReleaseEn.includes(value));
  const normalizedPressReleaseEn = pressReleaseEn.replace(/\r\n/g, '\n');
  assert(normalizedPressReleaseEn.includes('September 15, 2026\n\nFor Media\n\nBogosorGames'));
  assert(normalizedPressReleaseEn.includes(['Title: NyctoType','Developer: BogosorGames','Genre: Strategic typing game','Platform: PC (Windows) / Steam','Players: 1-2 players (online multiplayer)','Planned release: Early Access planned for 2027'].join('\n\n')));
