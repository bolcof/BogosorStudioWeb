  ...
  <main>
    ...
    <div class="wrap press-layout">
      ...
      <div class="press-main">
        ${section('about',bi(data.short,'p') + bi(labels.rules,'h3') + bi(data.rulesSummary,'p') + bi(labels.features,'h3') + data.features.map(f=>`<div class="press-feature">${bi(f.title,'h4')}${bi(f.text,'p')}</div>`).join('') + `<div class="press-planned">${bi(labels.planned,'h3')}${bi(data.planned,'p')}</div>`)}
