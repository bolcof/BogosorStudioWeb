...
function Slot({ name, showFlow = false }: { name: SlotKey; showFlow?: boolean }) {
  const slot = imageSlots[name];
  return <figure className="asset-slot" data-slot={slot.id}>
    {slot.src ? <a href={slot.href || slot.src} target="_blank" rel="noreferrer"><img className="slot-image" src={slot.src} alt={slot.title} width="1280" height="720" loading="lazy"/>{showFlow && <span className="rule-step-arrow" aria-hidden="true"/>}</a> : <div className="placeholder"><div className="placeholder-top"><span>IMAGE {slot.id}</span><ImageIcon size={22}/></div><div><strong>{slot.title}</strong><p>{slot.brief}</p></div><code>{slot.file}</code>{showFlow && <span className="rule-step-arrow" aria-hidden="true"/>}</div>}
